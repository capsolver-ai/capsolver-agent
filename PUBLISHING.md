...
This checklist is for maintainers releasing `capsolver-agent` from the official
...
```powershell
$version = "0.1.1"
$package = "capsolver-agent"
$module = "capsolver_agent"
$command = "capsolver-agent"
```
...
- `src/capsolver_agent/__init__.py` exposes the same `__version__`.
...
- The tree does not contain `.venv`, `uv.lock`, caches, local path overrides,
  real API keys, browser profiles, prompts, tool traces, or private service
  data.
